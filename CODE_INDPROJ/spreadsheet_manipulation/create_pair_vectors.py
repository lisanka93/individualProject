...
directory1 = 'FOLDER WITH ARGUMENT PAIRS'


#this one for individual arguments for look-up
directory2 = """FOLDER WITH INDIVIDUAL ARGUMENT DEBATES"""
...
for file in os.listdir(directory1):
	...
	for i in range(2,end+1,1):           #start in second row until end in one steps
		id1 = str(sheet1.cell(row = i, column =1).value)
		id2 = str(sheet1.cell(row = i, column =2).value)
		...
		for j in range(2,end2+1,1):
			value = str(sheet2.cell(row = j, column =1).value)
			print value
			if value == id1:
		...
		for k in range(2,end2+1,1):
			value = str(sheet2.cell(row = k, column =1).value)
			if value == id2:
